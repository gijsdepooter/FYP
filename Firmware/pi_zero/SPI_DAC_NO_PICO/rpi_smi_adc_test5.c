...
void *dac_thread_func(void *arg);

typedef struct {
    uint16_t val;
    int increment;
    volatile int adc_sampling_done; // Add this flag to indicate when ADC sampling is done
    pthread_mutex_t ready_mutex; // Add a mutex for the ready condition
    pthread_cond_t ready_cond; // Add a condition variable for the ready signal
    int dac_ready; // Add a flag to indicate DAC thread is ready
} dac_params;
...
int main(int argc, char *argv[])
{
    ...
    params.val = 0;
    params.increment = 0;
    params.adc_sampling_done = 0;
    pthread_mutex_init(&params.ready_mutex, NULL);
    pthread_cond_init(&params.ready_cond, NULL);
    params.dac_ready = 0;


    int ret = pthread_create(&dac_thread, NULL, dac_thread_func, (void *)&params);
    if (ret != 0) {
        perror("Error creating DAC thread");
        return 1;
    }
    ...
    while (!params.dac_ready) {
        pthread_cond_wait(&params.ready_cond, &params.ready_mutex);
    ...
    params.adc_sampling_done = 1;
    printf("DAC final: %d\n", params.val);
    ret = pthread_join(dac_thread, NULL);
    if (ret != 0) {
        perror("Error joining DAC thread");
        return 1;
    }
    pthread_mutex_destroy(&params.ready_mutex);
    pthread_cond_destroy(&params.ready_cond);
...
int adc_gpio_val(void)
{
    int gpio_lev = *REG32(gpio_regs, GPIO_LEV0);
    return((gpio_lev >> ADC_D0_PIN) & ((1 << ADC_NPINS) - 1));
}
...
void *dac_thread_func(void *arg) {
    dac_params *params = (dac_params *)arg;
    ...
    if (ioctl(fd, SPI_IOC_WR_BITS_PER_WORD, &bits) == -1 ||
        ...
        perror("SPI settings failed.");
        exit(1);
    }
    output_dac_value(fd, 0);

    pthread_mutex_lock(&params->ready_mutex);
    params->dac_ready = 1;
    pthread_cond_signal(&params->ready_cond);
    pthread_mutex_unlock(&params->ready_mutex);

    while (!params->adc_sampling_done) {
        params->val += params->increment;
        if (params->val >= 820) { // the maximum value for 12 bits
            params->val = 820;
        }
        output_dac_value(fd, params->val);
        usleep(5);
    }
